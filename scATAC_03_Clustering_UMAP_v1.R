...
wd <- "~/wjg/jgranja/project10x/analysis/2019/Re-Analysis/scATAC/clustering/heme/manual/"
setwd(wd)

se <- readRDS("../../../data/scATAC_Healthy_10x-Rename-190503.rds")
se2 <- readRDS("../output/LSI-Clustering/PC1.1.50.PC2.1.50.Peaks.50000.Resolution.15/PC1.1.50.PC2.1.50.Peaks.50000.Resolution.15.se.rds")
matSVD <- metadata(se2)$LSI[[1]]
clusters <- colData(se2)$Clusters
...
pdf("Plot_UMAP-NN-55-MD-45.pdf", width = 12, height = 12, useDingbats = TRUE)
df <- data.frame()
print(ggPlotPoint(
    x = uwotUmap[[1]][,1],
    y = uwotUmap[[1]][,2], 
...
colData(se)$initialClusters <- clusters
colData(se)$UMAP1 <- uwotUmap[[1]][,1]
colData(se)$UMAP2 <- uwotUmap[[1]][,2]

#save_uwot(uwotUmap, "scRNA-Hematopoiesis-UMAP-model.190505.uwot")
model <- uwotUmap
file <- "scATAC-Hematopoiesis-UMAP-model.190505.uwot.tar"
mod_dir <- tempfile(pattern = "dir")
dir.create(mod_dir)
uwot_dir <- file.path(mod_dir, "uwot")
dir.create(uwot_dir)
...
#seOld <- readRDS("../../../../../SE_Final/scRNA_Healthy_Heme.UMAP.LSI.Clusters.rds")
metadata(se)$variablePeaks <- paste(seqnames(se),start(se),end(se),sep="_")[metadata(se2)$LSIPeaks]
metadata(se)$LSI <- metadata(se2)$LSI
metadata(se)$matSVD <- metadata(se2)$LSI[[1]]
metadata(se)$UMAP_Model <- paste0(wd,file)
metadata(se)$UMAP_Params <- list(NN = 55, MD = 0.45, PCs = 1:50, VarPeaks = 50000, Res = "1.5")
saveRDS(se, "../../../data/scATAC_Healthy_10x-Rename-UMAP-190505.rds")
